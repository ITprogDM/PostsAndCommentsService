...
import (
	...
	mock_mode "OdinVOdin/tests/mode/mock"
	mock_service "OdinVOdin/tests/service/mock"
	"database/sql"
	"errors"
...
func TestCommentsService_CreateComment(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:      args{comment: defaultComment},
			want:      FuncRes{com: models.Comment{}, err: errors_project.ResponseError{}},
		...
		{
			name:       "Wrong Content",
			args:       args{comment: models.Comment{Author: "A", Content: string(make([]byte, constants.MaxContentLength+1))}},
			want:       FuncRes{com: models.Comment{}, err: errors_project.ResponseError{}},
			repoRes:    FuncRes{com: models.Comment{}, err: nil, skip: true},
			getterRes:  defaultPost,
			getterErr:  nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			repo := mock_mode.NewMockComments(ctl)
			...
			c := service.NewCommentsService(repo, logger, getter)
